...
Let $N$ be a sufficiently large integer. Let $K,L>0$ be reals such that $0<K\leq N$. Let $k$ be an integer such that $1\leq k \leq N/64$.  Let $A\subset [1,N]$ be a finite set of integers such that
\begin{enumerate}
...
For any $h\in\mathfrak{m}_2$, let $I_h$ be the interval of length $K$ centred at $kh$. Since $h\not\in\mathfrak{m}_1$, condition 2(a) cannot hold, and therefore there is some $x\in I_h$ divisible by all $q\in\mathcal{D}_{I_h}(A;L)$. In particular, $kh$ is distance at most $K/2$ from some multiple of $[\mathcal{D}_{I_h}(A;L)]$.

Therefore, for any $\mathcal{D}\subset \mathcal{Q}$, the number of $h\in\mathfrak{m}_2$ with $\mathcal{D}_{I_h}(A;L)=\mathcal{D}$ is at most $K$ times the number of multiples of $[\mathcal{D}]$ in $[1,k[A]+K]$. The number of multiples of $[\mathcal{D}]$ in $[1,k[A]+K]$ is at most $(k[A]/[\mathcal{D}])+K$, and hence the number of $h\in\mathfrak{m}_2$ with $\mathcal{D}_{I_h}(A;L)=\mathcal{D}$ is at most
\[Kk\frac{[A]}{[\mathcal{D}]} +K\leq Kk\prod_{q\in\mathcal{Q}\backslash\mathcal{D}}q\leq kN^{\abs{\mathcal{Q}\backslash \mathcal{D}}+1}+N.\]
In particular,
...
By definition of $\mathfrak{m}$, if $h\in\mathfrak{m}_2$ then $kh$ is distance greater than $K/2$ from any multiple of $[A]$, and hence $\mathcal{D}_{I_h}(A;L)\neq \mathcal{Q}$. Therefore (using the trivial estimate $\abs{\mathcal{Q}}\leq N$)
\begin{align*}
\sum_{h\in\mathfrak{m}_2}C(A;h,k)
&\leq
2kN\sum_{\mathcal{D}\subsetneq \mathcal{Q}}N^{-3\abs{\mathcal{Q}\backslash \mathcal{D}}}\\
&\leq \frac{2k}{N}(1+1/N)^{\abs{\mathcal{Q}}}\\
&\leq 8k/N \leq 1/8.
\end{align*}
\end{proof}
...
\begin{proposition}\label{prop:fourier}
  ...
  There exists a constant $c>0$ such that the following holds. Suppose that $N$ is sufficiently large. Let $K,L,M$ be reals such that $K<M\leq N$ and $k$ an integer such that $1\leq k\leq N/64$. Let $A\subset [M,N]$ be a set of integers such that
\begin{enumerate}
...
If the conclusion fails, there is an immediate contradiction by combining Lemmas~\ref{lem:minor_lbound}, \ref{lem:minor1_bound} and \ref{lem:minor2_bound} (and the required upper bound $[A]<2^{\abs{A}-1}$ comes from Lemma~\ref{lem:smooth_lcm}, provided the constant $c>0$ is chosen to be sufficiently small ).
\end{proof}
