...
lemma nat_cast_diff_issue_standalone {x y : ℤ} : (|x-y|:ℝ) = int.nat_abs (x-y) := sorry
...
lemma sub_le_omega_div_standalone {a b : ℕ} (h: b ∣ a) : (ω a:ℝ) - ω b ≤ ω (a/b) := sorry

lemma omega_div_le_standalone {a b : ℕ}  (h : b ∣ a) : ω (a/b) ≤ ω a := sorry

lemma omega_mul_ppower_standalone {a q : ℕ} (hq : is_prime_pow q) : ω (q*a) ≤ 1 + ω a := sorry
